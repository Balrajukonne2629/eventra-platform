...
export default function RegisterModal({ event, onClose }) {
  // Single source of truth — read directly from global auth context
  const { user } = useAuth();
  ...
  const handleSubmit = async (e) => {
    ...
    const normalizedTeamName =
      event?.teamSize > 1
        ? teamName.trim()
        : `${user?.name || "Individual"}-${String(userId).slice(-6)}`;
        ...
        <div className="p-6 sm:p-8">
          ...
          {success ? (
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              {/* Read directly from global user — no local copy needed */}
              <InputField
                label="Student Name"
                value={user?.name || ""}
                onChange={() => {}}
              ...
              {event?.teamSize > 1 && (
                <div className="mt-2">
                  ...
                  <p className="mt-2 text-xs font-medium italic text-slate-400">
                    Use the same team name to join an existing team.
                  </p>
                </div>
              )}
