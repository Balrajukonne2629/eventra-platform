...
export default function EventCard({
  ...
  const deadlineTime = new Date(event.deadline).toLocaleTimeString([], {
    hour: "2-digit",
    minute: "2-digit",
  });
  const externalFormLink =
    typeof event.externalFormLink === "string"
      ? event.externalFormLink.trim()
      : "";
    ...
    <div className="surface-card rounded-2xl p-6 transition-all duration-300 hover:-translate-y-1 hover:border-slate-500/70 flex h-full flex-col">
      <div className="flex flex-grow flex-col gap-3.5">
        <div className="flex justify-between items-start">
          <h3 className="line-clamp-2 text-2xl font-bold leading-tight text-slate-50">
            {event.title}
          </h3>
        </div>
        ...
        <div className="flex gap-2 items-center flex-wrap my-1">
          <span
            className={`px-3 py-1 rounded-full text-[11px] font-semibold uppercase tracking-wider whitespace-nowrap border ${event.category === "Technical" ? "bg-blue-500/20 text-blue-200 border-blue-400/30" : "bg-fuchsia-500/20 text-fuchsia-200 border-fuchsia-400/30"}`}
          >
            {event.category}
          </span>
